...
jenkins_folder = instance.getItem("Jenkins")
if(!jenkins_folder) {
  jenkins_folder = j.createProject(com.cloudbees.hudson.plugins.folder.Folder, "Jenkins")
  seed_job_dir = new File(home + '/init.dsl.d')
  if(seed_job_dir.exists() && seed_job_dir.isDirectory()) {
    def seed = jenkins_folder.createProject(FreeStyleProject, 'init-dsl')
    seed.buildersList.add(
        new javaposse.jobdsl.plugin.ExecuteDslScripts(
          new javaposse.jobdsl.plugin.ExecuteDslScripts.ScriptLocation('false', '*.groovy', null),
          true,
          javaposse.jobdsl.plugin.RemovedJobAction.IGNORE,
          javaposse.jobdsl.plugin.LookupStrategy.JENKINS_ROOT
        )
      )
      seed.save()
      def ws = instance.getWorkspaceFor(seed)
      new hudson.FilePath(seed_job_dir).copyRecursiveTo(ws)
      seed.addTrigger(new org.jvnet.hudson.plugins.triggers.startup.HudsonStartupTrigger("master", null, null))
  }
}
jenkins_folder.save()
instance.save()
